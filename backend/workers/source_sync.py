...
async def sync_daily_data_with_source(
    ...
    logger.info(f"Sync complete for {trade_date}: {total_count} market records, {results['valuation_count']} valuation records")
    ...
    for i, trade_date in enumerate(trading_days):
        if progress_callback:
            await progress_callback(
                ...
                {"action": "backfill", "date": str(trade_date), "progress": f"{i + 1}/{len(trading_days)}"}
            )
