...
class ScoringEngine:
    ...
    def calculate_super_trend_score(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        Iter 14: Removed close_strength (harmful). Added elg_net/mf_net percentile
        signals. Boosted volume_consistency. Removed buildup (redundant with VCS).
        """
        if df.is_empty():
            return df
        ...
        df = df.with_columns(
            [
                ...
                (pl.col("mf_net_percentile").fill_null(50.0)).alias("mf_flow_component"),
                ((-pl.col("sector_momentum_5d").fill_null(0.0).clip(-5.0, 0.0)) / 5.0 * 100)
                .fill_null(0.0)
                .alias("sector_weakness_penalty"),
            ]
        ...
        raw_score = (
            pl.col("accum_component") * 0.02
            + pl.col("alignment_component") * 0.10
            + pl.col("trend_quality_component") * 0.10
            + pl.col("consistency_component") * 0.08
            + pl.col("anti_climax_component") * 0.15
            + pl.col("consolidation_component") * 0.10
            + pl.col("momentum_quality_component") * 0.10
            + pl.col("elg_flow_component") * 0.08
            + pl.col("mf_flow_component") * 0.05
            - pl.col("recent_spike_penalty") * 0.10
            - pl.col("resistance_penalty") * 0.10
    ...
    def calculate_main_rally_score(self, df: pl.DataFrame) -> pl.DataFrame:
        """Calculate main rally (主升浪) score.
    ...
    def calculate_dragon_leader_score(self, df: pl.DataFrame) -> pl.DataFrame:
        """Calculate dragon leader (龙头先锋) score.
