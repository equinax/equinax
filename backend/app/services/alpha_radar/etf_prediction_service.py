...
class ETFPredictionService:
    ...
    async def get_predictions(
        ...
        # Calculate scores
        scores_df = self._calculate_scores(subcategory_df, df, target_date)
    ...
    def _calculate_scores(
        ...
        target_df = target_df.with_columns([
            (pl.col("flow_proxy") / pl.col("flow_5d_avg").abs().clip(lower_bound=1e6))
            .alias("flow_ratio"),
        ])

        # Calculate volume percentile (position in 60-day range)
        target_df = target_df.with_columns([
            (
        ...
        target_df = target_df.with_columns([
            (pl.col("change_5d").rank() / n_subcats).alias("price_rank"),
    ...
    def _calculate_activation_scores(
        ...
        # Join and calculate activation score
        activation_df = small_etf_perf.join(
            subcat_changes,
            on="sub_category",
            how="left",
        )
    ...
    def _build_predictions(
        ...
        for row in scores_df.iter_rows(named=True):
            ...
            predictions.append({
                ...
                "change_5d": self._to_decimal(row.get("change_5d")),
    ...
    def _generate_signals(self, row: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if price_rank < 0.3 and flow_rank > 0.7:
            signals.append({
                ...
                "description": f"5日涨幅排名后30%，但资金流入排名前30%，价量背离明显",
            })
