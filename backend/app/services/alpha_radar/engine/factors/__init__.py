...
def compute_all_factors(df: pl.DataFrame) -> pl.DataFrame:
    ...
    df = compute_climax(df)                     # needs vol_ma20, price_position_60d, upper_shadow_ratio, vol_peak_today_20d
    df = compute_main_strength(df)              # needs vol_ramp_5v20, vol_cv_10d, up_volume_ratio, turnover_change_20d
    df = compute_resistance(df)                 # needs price_range_position_20d, _ret_5d, vol_ramp_5v20, vol_jump_1d
    
    # Phase 4: Cleanup intermediates
    df = df.drop([
        "up_volume_20d", "total_volume_20d", "ma_10", "ma_20", 
        "vol_ma20", "max_loss_20d",
