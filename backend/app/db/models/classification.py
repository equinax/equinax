...
class StockStructuralInfo(Base):
    ...
    code: Mapped[str] = mapped_column(
        String(20), ForeignKey("asset_meta.code", ondelete="CASCADE"), primary_key=True
    )

    # Board and structural type
    board: Mapped[BoardType] = mapped_column(String(20), nullable=False)  # MAIN/GEM/STAR/BSE
    structural_type: Mapped[StructuralType] = mapped_column(
        String(20), nullable=False
    )  # NORMAL/ST/ST_STAR/NEW

    # Price limits
    price_limit_up: Mapped[Decimal] = mapped_column(
        Numeric(5, 2), nullable=False, default=10
    )  # 涨停幅度 10/20/30/5
    price_limit_down: Mapped[Decimal] = mapped_column(
        Numeric(5, 2), nullable=False, default=10
    )  # 跌停幅度
...
class StockIndustryMapping(Base):
    ...
    classification_system: Mapped[str] = mapped_column(
        String(20), nullable=False, default="sw"
    )  # sw/em/csrc
    ...
    __table_args__ = (
        PrimaryKeyConstraint(
            "stock_code", "industry_code", "classification_system", "effective_date"
        ),
...
class StockStyleExposure(Base):
    ...
    avg_turnover_20d: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(8, 4), nullable=True
    )  # 20日平均换手率
    ...
    # Value factor (估值因子)
    ep_ratio: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(8, 4), nullable=True
    )  # E/P (PE的倒数)
    bp_ratio: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(8, 4), nullable=True
    )  # B/P (PB的倒数)
    ...
    momentum_20d: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(8, 4), nullable=True
    )  # 20日收益率
    momentum_60d: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(8, 4), nullable=True
    )  # 60日收益率
...
class MarketRegime(Base):
    ...
    regime_score: Mapped[Optional[Decimal]] = mapped_column(
    ...
    avg_turnover: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(8, 4), nullable=True
    )  # 平均换手率
    ...
    __table_args__ = (Index("idx_market_regime_regime", "regime"),)
