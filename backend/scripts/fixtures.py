...
def extract_stock_data(
    ...
    target_conn.executemany(
        "INSERT OR REPLACE INTO daily_k_data VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", rows
    )
...
def extract_index_data(source_path: Path, target_path: Path) -> int:
    ...
    if rows:
        target_conn.executemany(
            "INSERT OR REPLACE INTO index_constituents VALUES (?,?,?,?,?,?,?)", rows
        )
...
def extract_industry_data(source_path: Path, target_path: Path) -> int:
    ...
    try:
        ...
        if rows:
            target_conn.executemany(
                "INSERT OR REPLACE INTO industry_classification VALUES (?,?,?,?,?,?)", rows
            )
    ...
    try:
        ...
        if rows:
            target_conn.executemany(
                "INSERT OR REPLACE INTO stock_industry_mapping VALUES (?,?,?,?,?,?)", rows
            )
...
def download_market_cap_fixture(trading_dates: list) -> int:
    """DEPRECATED: akshare has been removed. Use TuShare daily_basic for valuation data."""
    console.print(
        "\n[yellow]DEPRECATED: Market cap fixture download is no longer available.[/yellow]"
    )
    console.print(
        "[dim]akshare has been removed from the project. Use TuShare daily_basic instead.[/dim]"
    )
    return 0


def download_northbound_fixture(trading_dates: list) -> int:
    """DEPRECATED: akshare has been removed."""
    console.print(
        "\n[yellow]DEPRECATED: Northbound fixture download is no longer available.[/yellow]"
    )
    console.print("[dim]akshare has been removed from the project.[/dim]")
    return 0
...
def download_institutional_fixture() -> int:
    """DEPRECATED: akshare has been removed."""
    console.print(
        "\n[yellow]DEPRECATED: Institutional fixture download is no longer available.[/yellow]"
    )
    console.print("[dim]akshare has been removed from the project.[/dim]")
    return 0
...
def generate(
    ...
    if stock_dbs:
        ...
        if latest_date:
            ...
            all_codes = list(set(HS300_SAMPLES[:30] + ZZ500_SAMPLES[:30] + ZZ1000_SAMPLES[:20]))[
                :stocks
            ]

            target_path = FIXTURES_DIR / "sample_stocks.db"
            count = extract_stock_data(
                latest_stock_db, target_path, all_codes, start_date, end_date
            )
            stats["stocks"] = count
            if target_path.exists():
                console.print(
                    f"Created: {target_path.name} ({target_path.stat().st_size / 1024 / 1024:.2f} MB)"
                )
    else:
...
@app.command()
def download(
    fixture_type: str = typer.Argument(
        "all", help="Fixture type: market_cap, northbound, institutional, all"
    ),
):
    """
    ...
    console.print(
        f"Trading dates: {trading_dates[0]} to {trading_dates[-1]} ({len(trading_dates)} days)"
    )
