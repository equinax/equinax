...
function MarketAbstainBanner({ reason, dashboard }: { reason?: string; dashboard?: DashboardResponse }) {
  const reasonText: Record<string, string> = {
    regime_too_hot: '市场情绪偏热，隔夜策略胜率下降',
    market_hostile: '市场处于高风险状态',
    narrow_breadth: '市场广度不足',
  }

  const adviceText: Record<string, string> = {
    regime_too_hot: '建议观望，等待市场降温后再操作',
    market_hostile: '建议观望或适当减仓',
    narrow_breadth: '建议等待市场广度恢复',
  }

  const displayReason = reason ? (reasonText[reason] || reason) : '市场环境不适合选股'
  const advice = reason ? (adviceText[reason] || '建议观望') : '建议观望'

  return (
    <div className="flex items-center gap-3 px-4 py-2.5 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-md mb-2">
      <ShieldAlert className="h-4 w-4 text-amber-600 dark:text-amber-400 shrink-0" />
      <div className="flex items-center gap-2 text-sm min-w-0">
        <span className="font-medium text-amber-800 dark:text-amber-200 shrink-0">⚠️ {displayReason}</span>
        <span className="text-amber-700/70 dark:text-amber-300/70 hidden sm:inline">—</span>
        <span className="text-amber-700/70 dark:text-amber-300/70 truncate hidden sm:inline">{advice}</span>
      </div>
      {dashboard && (
        <div className="ml-auto flex items-center gap-3 text-xs text-muted-foreground shrink-0">
          <span>涨跌 <span className="text-profit">{dashboard.market_breadth.up_count}</span>/<span className="text-loss">{dashboard.market_breadth.down_count}</span></span>
          <span>涨停 <span className="text-profit">{dashboard.market_breadth.limit_up_count || 0}</span>/<span className="text-loss">{dashboard.market_breadth.limit_down_count || 0}</span></span>
        </div>
      )}
    </div>
  )
...
export function RadarDataTable({
  ...
  return (
    <div>
      {abstain && data.length > 0 && (
        <MarketAbstainBanner reason={abstainReason} dashboard={dashboard} />
      )}
